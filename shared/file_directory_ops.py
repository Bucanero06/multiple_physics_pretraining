...
def move_files_in_batches(source_pattern, dest_dir, batch_size=1000, replace_existing=False):
    """
    Move files matching the source_pattern to dest_dir in batches.

    Parameters:
    - source_pattern (str): The file pattern to match files (e.g., /path/to/dir/*.rst).
    - dest_dir (str): The directory to move files to.
    - batch_size (int): The number of files to move in each batch.
    - replace_existing (bool): Whether to replace existing files in the destination directory.
    """
    ...
    for i in range(0, total_files, batch_size):
        ...
        for file in batch_files:
            ...
            counter += 1
        print(f"Moved {counter} files so far...")
