...
from shared.file_directory_ops import change_directory_manager, delete_files_or_directories, make_directory, copy_to, \
    validate_directory_and_get_full_path, move_files_in_batches
...
def initialize_sphinx(source_dir, project_name, author_name, version='0.0.1'):
    ...
    with change_directory_manager(source_dir):
        execute_command(f"sphinx-quickstart"
                        ...
                        f"--suffix .rst --master index "
                        f" --batchfile --sep --dot _"
                        )
...
def update_conf_py(documentation_dir, source_dir):
    ...
    with open(conf_py_path, 'w') as f:
        ...
        # Adding source_dir and subdirectories to sys.path
        ...
        #         f.write(f"sys.path.insert(0, os.path.abspath('{full_path}'))\n")

        # f.write(f"\nsys.path.insert(0, os.path.abspath('multiple_physics_pretrained_env/lib/python3.10/site-packages'))\n")
...
def build_sphinx_docs(documentation_dir, venv_path):
    try:
        activate_and_build_command = (
            f". {venv_path}/bin/activate && "
            f"export PYTHONPATH={venv_path}/lib/python3.10/site-packages:$PYTHONPATH && "
        ...
        with change_directory_manager(documentation_dir):
            execute_command(activate_and_build_command)
...
def main(project_name, author_name, source_dir, documentation_dir, exclude_dirs=None, remove_top_package_name=None,
         delete_old_files=False, venv_path=None):
    ...
    move_files_in_batches(f"{documentation_dir}/*.rst", f"{documentation_dir}/source/",
                          batch_size=1000, replace_existing=True)


    build_sphinx_docs(documentation_dir, venv_path=venv_path)
...
if __name__ == '__main__':
    ...
    #   python3.10 -m autodocumentation_python -p "Multiple Physics Pretrained" -a "" -s '.' -d docs -e multiple_physics_pretrained_env,docs  -r

    # main("Multiple Physics Pretrained", "", ".", "docs", "multiple_physics_pretrained_env,docs", None, True)
